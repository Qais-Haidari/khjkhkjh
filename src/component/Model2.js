...
export default function Model() {
  ...
  const [text, settext] = useState("Request (3)");
  ...
  const Submit = () => {
    const a1 = localStorage.getItem("date1");
    const a2 = localStorage.getItem("date2");
    const a3 = localStorage.getItem("date3");
    if (
      !Way ||
      time === "00:00pm" ||
      time === "00:00pm" ||
