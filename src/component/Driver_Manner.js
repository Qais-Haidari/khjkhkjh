...
export default function Driver_Manner() {
  ...
  const ratingChanged = (newRating) => {
    if (newRating < 3) {
      ...
      axios
        .put(
          `https://111.125.159.243:5000/Star/${localStorage.getItem("Car_Id1")}`
        )
        .then((res) => {
          setsubmit("Liked");
    ...
    if (!Com) {
      ...
      axios
        .post("https://111.125.159.243:5000/Command", {
          ...
          did: localStorage.getItem("Car_Id1"),
          good: Com,
          name: localStorage.getItem("EmployeeName"),
          phone: localStorage.getItem("PhoneNumber"),
          ephone: localStorage.getItem("Driver_Number1"),
        })
        .then((res) => {
